...
if [ -z ${CONDA_PATH+x} ]; then
  ...
  echo "./install.sh --env-name=sutherland $git_method --branch=tulio/transport"
  ./install.sh --env-name=sutherland $git_method --branch=tulio/transport
else
  echo "Using existing Conda installation, ${CONDA_PATH}"
  echo "./install.sh --conda-prefix=$CONDA_PATH --env-name=sutherland $git_method --branch=tulio/transport"
  ./install.sh --conda-prefix=$CONDA_PATH --env-name=sutherland $git_method --branch=tulio/transport
fi
