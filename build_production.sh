...
if [ -z ${CONDA_PATH+x} ]; then
  ...
  echo "./install.sh --env-name=production $git_method --branch=production"
  ./install.sh --env-name=production $git_method --branch=production
else
  echo "Using existing Conda installation, ${CONDA_PATH}"
  echo "./install.sh --conda-prefix=$CONDA_PATH --env-name=production $git_method --branch=production"
  ./install.sh --conda-prefix=$CONDA_PATH --env-name=production $git_method --branch=production
fi
